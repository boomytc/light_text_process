...
def _ensure_package_root():
    current = Path(__file__).resolve()
    for parent in current.parents:
        if parent.name == "fun_text_processing":
...
class Normalizer:
    ...
    def __init__(
        ...
        if NLP_AVAILABLE:
            ...
            self.processor = None
            LOGGER.info("NeMo NLP is not available. Moses de-tokenization will be skipped.")
    ...
    def process_batch(self, batch, verbose, punct_pre_process, punct_post_process):
        ...
        normalized_lines = [
            ...
            for text in tqdm(batch, disable=not verbose)
        ]
    ...
    def normalize(
        ...
        if punct_post_process:
            ...
            if self.processor:
                ...
                LOGGER.info("DAMO_NLP collection is not available: skipping punctuation post_processing")
