...
class MoneyFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, decimal: GraphFst):
        ...
        minor_number = pynini.union(
            pynutil.add_weight(((DAMO_SIGMA - "one") @ cardinal_graph), -0.7)
            @ add_leading_zero_to_double_digit,
            pynini.cross("one", "01"),
        )
        cents_unit = pynutil.delete("cents") | pynutil.delete("cent")
        pence_unit = (
            pynutil.delete("pence")
            | pynutil.delete("penny")
            | pynutil.delete("pennies")
        )
        ...
        graph_integer = (
            ...
            + delete_extra_space
            + graph_unit_plural
            + (optional_cents_standalone | optional_pence_standalone | optional_cents_suffix)
        )
        graph_integer |= (
            pynutil.insert('integer_part: "')
            + pynini.cross("one", "1")
